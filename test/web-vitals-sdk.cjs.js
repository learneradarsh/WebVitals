...
class WebVitalsSDK {
    ...
    trackLCP() {
        webVitals.onLCP((metric) => this.sendMetric('LCP', metric.value, metric.id));
    }
    trackFID() {
        webVitals.onFID((metric) => this.sendMetric('FID', metric.value, metric.id));
    }
    trackCLS() {
        webVitals.onCLS((metric) => this.sendMetric('CLS', metric.value, metric.id));
    }
}

exports.WebVitalsSDK = WebVitalsSDK;
